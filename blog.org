...
skeleton for Sphinx style docstrings does exist: [[https://github.com/naiquevin/sphinx-doc.el][sphinx-doc.el]]; but it
...
docstring. There's also the [[https://github.com/jcs-elpa/docstr][docstr]] package, which caters to any
...
function signature and body are parsed to determine argument names,
types, default values, return type and what kind of (if any)
exceptions can be raised. That information is used to generate the
=Parameters= block and the =Returns= block for the docstring. If
exceptions are found the =Raises= block is also added. The default
behavior prompts the user to enter a short and long function
...
example usage. Checkout the GitHub page to see a gif of the package in
action.

The package is [[https://melpa.org/#/numpydoc.el][available on MELPA]]. Just add to your (=use-package=
leveraging) =init.el=:
