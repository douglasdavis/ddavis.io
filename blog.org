...
The backend implementation is in C++ and accelerated with [[https://www.openmp.org/][OpenMP]],
with [[https://github.com/pybind/pybind11][pybind11]] used to generate the Python bindings.
...
NumPy (for a 2D histogram of 100 million $(x_i, y_i)$ pairs). The
...
each bin is a "first class citizen" in pygram11 (see my [[https://ddavis.io/posts/numpy-histograms/][NumPy
Histogram tricks for HEP]] post).
...
(checkout some [[https://pygram11.readthedocs.io/en/stable/bench.html][benchmarks]] in the documentation). To try it out,
...
** Avoiding conversions & boilerplate in pybind11      :cpp:python:tmp:numpy:
:PROPERTIES:
